<template>
    <div>
        ...
        <div class="flex flex-col md:flex-row md:max-h-80vh mt-12">
            <div
            ...
            <div class="w-full md:w-1/2 flex flex-col pr-8" id="return-reasons">
                ...
                <div class="mb-2 px-4">
                    ...
                    <p class="text-gray-500">
                        <span
                            ...
                            v-if="currentItem.sku"
                            >{{ currentItem.sku }}</span
                        >
                        <span>{{ order.currency }}{{ currentItem.price }}</span>
                    </p>
                </div>
                <ul class="px-2 py-4 max-w-lg">
                    <li
                        ...
                        class="p-4 text-lg border my-2 border-gray-200 hover:bg-gray-200 cursor-pointer"
                    >
                        {{ item.label }}
                    </li>
                </ul>
...
export default {
    setup(props) {
        ...
        const goBack = () => {
            if (currentItem.transaction_type == "return") {
                Inertia.get(`/${usePage().props.value.prefix}/orders/review`);
            } else {
        ...
        for (const index in surveys) {
            ...
            if (
                ...
                surveyItems = survey.items.filter(
                    (_item) => _item.survey_step_id == surveyStep.id
                );
                ...
                if (isAdmin) {
                    surveyItems.push({
                        id: 0,
                        survey_id: 0,
                        label: "Requested By CX",
                        value: "Requested By CX",
                        type: "radio",
                    });
                }
            }
        ...
        const selectSurveyItem = (item) => {
            ...
            if (nextSurveyItems()) {
                Inertia.get(`/${usePage().props.value.prefix}/requests/step/4`);
            } else {
                for (const index in order.line_items) {
                    const _item = order.line_items[index];
                    if (_item.key == currentItem.key) {
